...
void TraceBytes(U8 * Ptr, int Count); //These go out on Serial1
void TraceByte(U8 Value);             //This goes out on Serial1
...
const int8_t SENSEA = 36;  //pin 3
const int8_t SENSEB = 39;  //pin 4
...
//Note: comment it out to make inactive, don't define it as false
#define LINK_ACTIVE true
#ifdef LINK_ACTIVE
#define Trace(A)  WriteTraceBuffer(A)
#define Traceln(A) WriteTraceBufferLn(A);
#else
...
bool InitOffset()
{
    ...
    CSCue = -1;
    CSStep = 0;
    CSOffsetValsX10[0] = NOM_OFFSET_X10; //just in case, set a nominal value
    CSOffsetValsX10[1] = NOM_OFFSET_X10; //just in case, set a nominal value
 ...
 void ProcessCommand(String & Header) //Set or clear bits in the seven byte array for PQ driver board
{
    ...
    Trace("ProcessCommand: ");
    Traceln(Header.c_str());
        ...
        switch(BAttrs[Index])
        {
            ...
            case ATTR_CTLB:
                switch(Index)
                {
                    ...
                    case CMD_CONT_IDX:  //Continuity or Ohm button
                        sprintf(StringBuff, "Cmd: cont, %d", Index);
                        if(CtrlState == ST_IDLE)
                        {
                            //Clears all SR bits, selects Cue0 and TestCurrent,
                            //and shifts out to shift registers on power board
                            //also changes state to ST_CONT_SCAN
                            InitContinuityScan();
                        }
                    ...
                    case CMD_PWM1_IDX:
                        //This was to test PWMing of selected output at one setting
                        break;

                    case CMD_PWM2_IDX:
                        //This was to test PWMing of selected output at another setting
                        break;
...
void GenButton(String &Text, int ButtonIdx, bool On)
{
    ...
    //Everything above sets up ButtonClass string  and ButtonString (the text displayed centered in button)
      ...
      {             // if there's bytes to read from the client,
        ...
        {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0)
          {
            ...
            PageStr += "text-decoration: none; font-size: 8px; margin: 2px; cursor: pointer;}\r\n";
            ...
            PageStr += "</style></head>";
            client.println(PageStr); //send it out to client
            IdleServe();
            ...
            IdleServe();
            client.println("</body></html>");//send it out to client

            // The HTTP response ends with another blank line
            client.println(); //send it out to client
