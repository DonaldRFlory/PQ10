...
const int NUM_COMMANDS = 7; //keep this last and keep CmdStrs in line with cmd codes
...
bool GetCommandAndIndex(String &Header, int & Index, int & Command)
{
    ...
    for(i = 0; i < NUM_COMMANDS; ++i)
    {
        ...
        {
            j += sizeof(CommandString); //skip index over the Command to start of button number
            Command = i;
            Found = true;
            break;
        }
...
void ContinuityScan()
{
    ...
    if(CtrlState == ST_CONT_SCAN)
    {
        ...
        if(CSStep >= 60)
        { //done with this cue
             ClrSRBit(CueToSRIdx(CSCue)); //clear the just read cue's low drive
            ...
            else
            {
                ...
                else
                {
                    CSStep = 0;
                    ++CSCue;
                    SetSRBit(CueToSRIdx(CSCue));//turn on low drive of next cue
                }
            }
    ...
    else if(IsSRBitSet(CTRL_BIT1))
    {  //when not doing continuity scan, we read ADC0, ADC3 and ADC6 (SENSEA-SENSE3)
        //each once a millisecond
...
void InitContinuityScan()
{
    ...
    for(int i = 0; i < NUM_SR_BITS; ++i)
    {
        ...
        {
            ClrSRBit(i);//clear all SR bits which do not have ATTR_CTRL attribute
        }
    }
    CSStep = 0;
    CSCue = 0;
...
void ProcessCommand(String & Header) //Set or clear bits in the seven byte array for PQ driver board
{
    ...
    if(GetCommandAndIndex(Header, Index, Command))
    {
        ...
        {
            case CMD_OFF:
                //So I think I am allowing clear command for most bits only in ST_IDLE,
