...
bool InitDeviceCommunication(HANDLE Handle)
{
	...
	//for now we only have one comm type, serial and a HandleTable[Index].MaxSendSize = Size;
	...
	//	return false;
	//}
	//In this initial implementation for ESP32 we are requiring only the FIdx in packet and
	//using one length byte to account for parameters. So total raw packet is 1-256 bytes.
	//Total packet on serial comm is LB + (raw packet) + 2 CRC-Bytes  =  4-259 bytes.
	HandleTable[Index].MaxSendSize = 256;
	//Return always has status byte plus up to 255 return bytes for raw return 1-256 bytes
	//Wrapper (LB, 2_CRC_bytes) is 3 bytes so on bus 4-259 bytes
	HandleTable[Index].MaxReturnSize = 256;
	return true;
}
